document.addEventListener("DOMContentLoaded", function () {
  ...
  const tl = gsap.timeline({
    scrollTrigger: {
      ...
      onUpdate: (self) => {
        // 방향: self.direction 값 (1: 다운, -1: 업)
        const currentIndex = Math.round(self.progress * (lis.length - 1)); // 현재 활성화된 인덱스 계산
        $('section.artwork ul.pager li')
          .removeClass("on")
          .eq(currentIndex)
          .addClass("on");
        $("section.artwork .num span").text(currentIndex + 1);
      },
    }
  });
